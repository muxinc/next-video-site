...
import iconAdaptive from './_images/icon-adaptive.svg';
import iconAI from './_images/icon-ai.svg';
import iconAnalytics from './_images/icon-analytics.svg';
import iconFormats from './_images/icon-formats.svg';
import iconPlaceholder from './_images/icon-placeholder.svg';
import iconPlayer from './_images/icon-player.svg';
import iconStorage from './_images/icon-storage.svg';
import iconStreaming from './_images/icon-streaming.svg';
import iconTimeline from './_images/icon-timeline.svg';
import logoBlob from './_images/logo-blob.svg';
import logoMux from './_images/logo-mux-small.svg';
import logoS3 from './_images/logo-s3.svg';
import nextVideo from './_images/next-video.svg';
...
export default function Page() {
  return (
    <main className="overflow-x-hidden px-20 py-50 sm:px-40 sm:py-100">
      <section className="mx-auto mb-150 max-w-780 text-center">
        ...
        <h1 className="mb-30 text-64 font-bold -tracking-2">{data.heroTitle}</h1>
        ...
        <div className="relative grid w-full grid-cols-2 gap-20 px-100 py-50 font-mono text-16 uppercase tracking-1">
          ...
          <Link
            href="./docs"
            className="flex h-60 items-center justify-center rounded-full border border-white bg-pink-dark-30 "
          >
            Documentation
          </Link>
        </div>
      ...
      <section className="mx-auto max-w-1180">
        <header className="mb-100 mt-150 text-center">
          <h2 className="text-48 font-bold -tracking-2">{data.featuresTitle}</h2>
        ...
        <ul className="grid gap-20" style={{ gridTemplateColumns: 'repeat(auto-fit, minmax(20rem, 1fr))' }}>
          {data.features.map((feature, featureIdx) => (
            ...
            >
              ...
              <h3 className="mb-20 text-24 font-bold leading-rel-170 -tracking-2 supports-subgrid:mb-0">
                {feature.title}
              </h3>
      ...
      <section className="mx-auto max-w-1180">
        <header className="mb-80 mt-200 text-center">
          <h2 className="text-48 font-bold -tracking-2">{data.getStartedTitle}</h2>
        </header>
        <div className="grid grid-flow-col grid-cols-2 grid-rows-[auto_auto] gap-x-20 gap-y-40">
          <h3 className="text-center text-24 font-bold -tracking-2">{data.getStartedVideoTitle}</h3>
          <Video src={data.getStartedVideo} className="flex overflow-hidden rounded-20" accentColor="#fa50b5" />
          <h3 className="text-center text-24 font-bold -tracking-2">{data.getStartedCodeTitle}</h3>
          <div className="rounded-20 border border-gray-28 bg-soft-light p-40">
      ...
      <section className="relative my-200">
        <GridBackground inset="200" />
        <div className="mx-auto flex max-w-980 items-center gap-100 rounded-20 bg-black px-100 py-60">
          <div>
            <h2 className="mb-30 text-36 font-bold -tracking-2">{data.infrastructureTitle}</h2>
